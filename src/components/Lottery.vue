...
<transition name="pop">
        <div v-if="result" class="result-overlay">
          ...
          <div class="result_card">
            <div class="win-banner">JACKPOT!</div>
...
export default {
  data() {
    return {
      ...
      hasDrawn: false,
      prizes: [
        { text: '1st Prize 🎉', percent: 10, color: '#FF6B6B' },
        { text: '2nd Prize 🎁', percent: 20, color: '#FFD93D' },
        { text: '3rd Prize 🎈', percent: 30, color: '#6BCB77' },
        { text: 'No Prize 🙂', percent: 40, color: '#4D96FF' }
      ]
    }
  },
...
.result-label { font-size: 16px; color: #B8860B; margin-bottom: 10px; letter-spacing: 2px; }
.result-text { font-size: 32px; font-weight: 900; color: #E74C3C; margin-bottom: 30px; }

.close-btn {
  background: #27AE60; color: white; border: none;
  padding: 12px 40px; font-weight: bold; border-radius: 50px;
  cursor: pointer; box-shadow: 0 4px 0 #1E8449;
}

/* 纸屑动画 */
.confetti-wrapper { position: absolute; width: 100%; height: 100%; overflow: hidden; top: 0; left: 0; }
.confetti-piece {
